...
│  Python HTTP 服务（端口 5000）                          │
│  接收 JSON → toRemoteJSON() 转为中文键名               │
│  INSERT INTO wearable_device_data (deviceId, data,     │
│    createTime) VALUES (?, ?, NOW())                    │
└──────────────────────┬───────────────────────────────┘
                       ↓
┌──────────────────────────────────────────────────────┐
...
│  wearable_device_data ── 体征数据（我们写入）           │
│    id        : 自增主键                                 │
│    deviceId  : 关联 wearable_device.id                  │
│    data      : JSON 格式体征数据                        │
│    createTime: 插入时间                                 │
└──────────────────────────────────────────────────────┘
```
...
1. **离线优先架构**：本地 JSON 文件先存，HTTPS 上传后发，断网不丢数据
2. **设备无关协议**：Veepoo SDK 通过功能汇总包自动适配不同设备的采集能力
3. **中文 JSON 映射**：SDK 采集的英文字段自动转为中文键名存入六元数据库
4. **自动风险分级**：血压数据写入时自动按 AHA 标准分级（正常/偏高/高血压1级/高血压2级/危急）
5. **外网可达**：通过六元配置的反向代理，手机无需连接公司 WiFi 即可上传数据
